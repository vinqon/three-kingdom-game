  ...
  for (const origin of Object.values(state.cities)) {
    ...
    for (const target of legalTargets(state, origin.id, 'attack')) {
      if (troops > target.troops) {
        candidates.push({ mode: 'attack', originCityId: origin.id, targetCityId: target.id, troops });
      }
    }
  }
  return candidates;
...
function attackScore(state: GameState, decision: AiDecision): number {
  const target = state.cities[decision.targetCityId];
  const remainingEnemyCapitals = Object.values(state.cities).filter(
    (city) => city.capitalOf && city.capitalOf !== state.turnFaction && city.owner !== state.turnFaction,
  );
...
function threatenedCapitalTransfer(state: GameState): AiDecision | undefined {
...
function frontlineTransfer(state: GameState): AiDecision | undefined {
  const isFrontline = (cityId: string): boolean =>
    state.cities[cityId].adjacentCityIds.some(
      (neighborId) => state.cities[neighborId].owner !== state.turnFaction,
    );

  const enemyCapitals = Object.values(state.cities)
    .filter((city) => city.capitalOf !== undefined
      && city.capitalOf !== state.turnFaction
      && city.owner !== state.turnFaction)
    .map((city) => city.id);

  function distanceToEnemyCapital(startId: string): number {
  ...
  const candidates: Array<AiDecision & { distance: number }> = [];
  for (const origin of Object.values(state.cities)) {
...
export function chooseAiAction(state: GameState): AiDecision | undefined {
  ...
  const rankedAttacks = winningAttacks(state).sort(
    (left, right) =>
      attackScore(state, right) - attackScore(state, left) ||
      left.targetCityId.localeCompare(right.targetCityId) ||
      left.originCityId.localeCompare(right.originCityId),
  );
  const decisiveCapitalAttack = rankedAttacks.find(
    (decision) => attackScore(state, decision) >= 10_000,
  );
  return decisiveCapitalAttack
    ?? threatenedCapitalTransfer(state)
    ?? rankedAttacks[0]
...
export function runAiTurn(state: GameState): { state: GameState; events: GameLogEntry[] } {
  ...
  return { state: next, events: next.log.slice(initialLogLength) };
}
