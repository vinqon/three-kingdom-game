import { FACTION_ORDER } from './scenario.ts';
import {
  GameRuleError,
  type ActionMode,
  type ActionResult,
...
function cityOrThrow(state: GameState, cityId: string): CityState {
  const city = state.cities[cityId];
  if (!city) throw new GameRuleError('CITY_NOT_FOUND');
  return city;
}

export function reinforceFaction(state: GameState, faction: FactionId): GameState {
...
export function beginFactionTurn(state: GameState): GameState {
...
export function legalTargets(
  ...
  return origin.adjacentCityIds
    .map((cityId) => state.cities[cityId])
    .filter((target) =>
      mode === 'transfer' ? target.owner === origin.owner : target.owner !== origin.owner,
    );
}
...
function validateMove(
  ...
  command: MoveCommand,
  mode: ActionMode,
): { origin: CityState; target: CityState } {
...
export function transfer(state: GameState, command: MoveCommand): ActionResult {
  ...
  return { state: evaluateStatus(next), message };
}
...
export function evaluateStatus(state: GameState): GameState {
  const next = cloneState(state);
  for (const faction of FACTION_ORDER) {
    const enemyCapitals = Object.values(next.cities).filter(
      (city) => city.capitalOf && city.capitalOf !== faction,
    );
...
function ownsCity(state: GameState, faction: FactionId): boolean {
